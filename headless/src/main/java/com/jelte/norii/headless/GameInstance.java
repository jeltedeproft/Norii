...
public class GameInstance {
	...
	/**
	 * Makes a choice for a player in this game instance Also checks to see if the
	 * round/game has ended as a result
	 *
	 * @param player The player making the choice
	 * @param choice The choice being made
	 */
	public void makeChoice(ConnectedClient player, Move choice) {
		if ((player1.getConnectedClient() != player) && (player2.getConnectedClient() != player))
		...
		if (player1.getConnectedClient() == player) {
			player1.makeChoice(choice);
			player2.getConnectedClient().getConnection().sendTCP(new Packets.OpponentChosen());
		} else if (player2.getConnectedClient() == player) {
			player2.makeChoice(choice);
			player1.getConnectedClient().getConnection().sendTCP(new Packets.OpponentChosen());
		}
		...
		if ((player1.getChoice() != null) && (player2.getChoice() != null)) {
			...
			player1.getConnectedClient().getConnection().sendTCP(roundResult);
			...
			player2.getConnectedClient().getConnection().sendTCP(roundResult);
