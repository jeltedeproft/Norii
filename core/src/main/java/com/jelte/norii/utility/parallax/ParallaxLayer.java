...
/**
 *
 * This is an abstract class that encapsulates the logic of a Parallaxly Scrolling layer that ParallaxBackground renders. It stores the ratio at which the layer has to scroll relatively. You can extend this class and implement its internal
 * modal and rendering logic. This has an advantage over using just TextureRegions as layers of ParallaxBackground. Also see {@link TextureRegionParallaxLayer} as it is the direct extension of this class and ready to use if you want to
 * render single texture region per layer repeatedly.
 * <p>
 * As an example, consider you need to render 20 colored dots per camera viewport width over background. The balls need to scroll with the world camera, but with the parallax effect . Now you can extend this class to create your own balls
 * layer with internal logic to maintain the balls(logically) and render them.
 *
 * @author Rahul Verma
 *
 */
public abstract class ParallaxLayer {
	...
	/**
	 *
	 * TileMode basically represents the way a layer has to repeat itself while scrolling. Parallax Scrolling can be performed infinitely in any direction if TileMode is set to <b>TileMode.repeat</b> in that direction. The layer will just
	 * keep on repeating itself. Specifying <b>TileMode.single</b> makes the layer render only once without repeating in a direction
	 *
	 * @author Rahul
	 *
	 */
	...
	/**
	 * returns the width of this layer. This width basically represents segment width of this layer after which it either repeats itself while rendering or just ceases to render further, depending upon the horizontal TileMode (see
	...
	/**
	 * returns the height of this layer. This height basically represents segment height of this layer after which it either repeats itself while rendering or just ceases to render further, depending upon the vertical TileMode (see
