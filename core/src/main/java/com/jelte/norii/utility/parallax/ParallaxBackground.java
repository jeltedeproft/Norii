...
import com.jelte.norii.utility.parallax.ParallaxLayer.TileMode;

/**
 * A ParallaxBacground encapsulates the logic to Render ParallaxLayers. It owns a collection of ParallaxLayers. These Layers a rendered on screen showing parallax effect based on scrolling characteristics of individual layers. This
 * rendering module does not owns any Batch ,you need to provide one in the draw method.
 * <p>
...
public class ParallaxBackground {
	...
	public void draw(OrthographicCamera worldCamera, Batch batch) {
		...
		for (int i = 0; i < layers.size; i++) {
			...
			float currentX = (layer.getTileModeX().equals(TileMode.single) ? 0 : ((int) ((worldCamera.position.x - (worldCamera.viewportWidth * .5f * worldCamera.zoom)) / layer.getWidth())) * layer.getWidth())
					- (Math.abs((1 - layer.getParallaxRatio().x) % 1) * worldCamera.viewportWidth * .5f);
			do {
				float currentY = (layer.getTileModeY().equals(TileMode.single) ? 0 : ((int) ((worldCamera.position.y - (worldCamera.viewportHeight * .5f * worldCamera.zoom)) / layer.getHeight())) * layer.getHeight())
