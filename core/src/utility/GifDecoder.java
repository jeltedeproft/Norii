...
public class GifDecoder {
	...
	private static class DixieMap extends Pixmap {
		...
		void getPixels(final int[] pixels, int offset, final int stride, final int x, final int y, final int width, final int height) {
			final java.nio.ByteBuffer bb = getPixels();
			...
			for (k = y; k < y + height; k++) {
				int _offset = offset;
				for (l = x; l < x + width; l++) {
					final int pxl = bb.getInt(4 * (l + k * width));
	...
	private static class GifFrame {
		public GifFrame(final DixieMap im, final int del) {
			image = im;
			delay = del;
		}
	...
	public Animation<TextureRegion> getAnimation(final PlayMode playMode) {
		final int nrFrames = getFrameCount();
		Pixmap frame = getFrame(0);
		final int width = frame.getWidth();
		final int height = frame.getHeight();
		int vzones = (int) Math.sqrt(nrFrames);
		final int hzones = vzones;
		...
		final Pixmap target = new Pixmap(width * hzones, height * vzones, Pixmap.Format.RGBA8888);

		for (h = 0; h < hzones; h++) {
			for (v = 0; v < vzones; v++) {
				final int frameID = v + h * vzones;
				if (frameID < nrFrames) {
		...
		final Array<TextureRegion> texReg = new Array<TextureRegion>();

		for (h = 0; h < hzones; h++) {
			for (v = 0; v < vzones; v++) {
				final int frameID = v + h * vzones;
				if (frameID < nrFrames) {
					final TextureRegion tr = new TextureRegion(texture, h * width, v * height, width, height);
					texReg.add(tr);
				}
			}
		}
		...
		final Animation<TextureRegion> result = new Animation<TextureRegion>(frameDuration, texReg, playMode);

		return result;
	}

	public static Animation<TextureRegion> loadGIFAnimation(final Animation.PlayMode playMode, final InputStream is) {
		final GifDecoder gdec = new GifDecoder();
		gdec.read(is);
